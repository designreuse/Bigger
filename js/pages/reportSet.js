...
reportSet.extend({
	...
	getChartSet : function() {
		...
		console.log(id);
		if (id == "#popular") {
			that.options.YKey.push($(id + ' #popularY input:checked')
					.attr("id"));
			...
			if (xtype == "#popular11") {
				...
				that.options.pinciTime.value = $(
						xtype + ' input[name="pinci_time"]:checked').val();
			}
			...
			// 获取y轴数据关键字
			that.options.YKey
					.push($(id + ' #recentY input:checked').attr("id"));
