...
.include([
		...
		'js/bootstrap-datetimepicker-master/js/bootstrap-datetimepicker.js',
		'js/bootstrap-datetimepicker-master/js/locales/bootstrap-datetimepicker.zh-CN.js' ]);
*/
...
function pageStart()
{
	...
	reportListDemo.getReport(function(data) {

		console.log(data);

		$.each(data, function(i, field) {
			if (field.chart_type == "pinciValue"
					|| field.chart_type == "pinciTime") {
				$("#pinci").append(
						[
		...
		$("#reportMenu ul li a").on('click', function() {
			...
			console.log(data.grid_column);
			 ...
			 var reportSets = new reportSet(options);
			  reportSets.createReport();

			};
			reportListDemo.options = $.extend({}, reportListDemo.options);
			reportListDemo.ajax();
	...
	$("#Search").click(function(){
		...
		var end =$("#datetimeEnd").val();
		   ...
		   var vehicles=$("#vehicles").select2("val");
		   console.log(vehicles)
		//var  vehicles=$("#vehicles").data();
		if(!vehicles)
			{
			vehicles=[ 'B10002' ];
		}
		 var tempOption=$.extend({},options,{
			
			 vehicles : vehicles,
			 date_from : start,
			 date_to : end
		});
		 var reportSets = new reportSet(tempOption);
		 reportSets.createReport();
