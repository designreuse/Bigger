...
; (function (define) {
    define(['jquery'], function ($) {
        ...
        var defaultSettings = {
            ...
            confirmLabel: 'OK',
            size: empty,
            loadingHtml: '<h5>Loading...</h5><div class="loading-overlay" style="background-color: rgb(253, 253, 253);"><div class="loader black"></div></div>',
            title: 'Attention'
        };
        ...
        function _getModalInstance() {
            ...
            function createModalElement() {
                ...
                + ' <div class=modal-header> <button type=button class="x close pull-right" data-dismiss=modal><span aria-hidden=true>&times;</span><span class=sr-only>Close</span></button><button type=button id=expend class=" expend  pull-right" ><span class="licon-size-fullscreen"></span></button> <h4 class=modal-title></h4></div>'
                + '</div>'
                + '</div>' 
                + '</div>')
                .on('hidden.bs.modal', _recycleModal)
                .on(eventClick, '#expend', function (ev) {
               var exp= 	$(this).data("expand");
                	if(!exp||exp=="expand")
                		{
                		$(this).data("expand","compress");
                var size=	$.pageSize();
                	 var width=size.pageW;
                	 var height=size.pageH;
                	 $("#iframeContent").innerHeight(height-$(".modal-header").outerHeight()-2);
                	 $(".modal-xl").width("100%").css("margin",0);
                	 $(".modal-xl").parent().css("padding",0);
                	 $(".licon-size-fullscreen").addClass("licon-size-actual").removeClass("licon-size-fullscreen");
                		}else
                			{
                			$(this).data("expand","expand");
                			$("#iframeContent").css("height","75vh");
                			$(".modal-xl").width("96%").css("margin","30px");
                			 $(".licon-size-actual").addClass("licon-size-fullscreen").removeClass("licon-size-actual");
                			}
        ...
        function iframe(params, title) {
            var html = ('<div class=modal-body style="position: absolute;width: 100%;background-color: rgba(255,255,255,0.8);height: 100%;">%1%</div>' +
                        '<iframe id="iframeContent" class="embed-responsive-item" frameborder=0 src="%0%" style="width:100%;height:75vh;display:block;"/>')
